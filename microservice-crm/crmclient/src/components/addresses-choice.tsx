...
export class AddressesChoice extends React.Component<
  IAddressProperty,
  Map<string, string>
> {
  ...
  public render() {
    return (
      <Form.Control as="select" onChange={this.Changed}>
  ...
  private Changed(optionSelected: any) {
    ...
    const address = Object.values(this.addresses).find(
      (x) => x.ServerID === optionSelected.target.value
    );
    if (address) {
  ...
  private LoadAddreses(): void {
    axios
      .get("http://localhost:17171/get/")
      .then((result: AxiosResponse<any>) => {
        result.data.forEach((person: any) => {
          this.options.set(
            person.ServerID,
            `${person.Line1} ${person.Line2} ${person.Line3} ${person.Line4} ${person.PostalCode}`
          );
        });
