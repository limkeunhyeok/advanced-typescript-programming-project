...
export class MapEvents {
  ...
  constructor(
    ...
    const subscription = this.pinsModel
      .Load()
      .subscribe((data: PinModelData[]) => {
        data.forEach((pinData) => {
          const pin: Microsoft.Maps.Pushpin = new Microsoft.Maps.Pushpin(
            new Microsoft.Maps.Location(pinData.lat, pinData.long),
            {
              draggable: true,
            }
          );
          ...
          const dragHandler = Microsoft.Maps.Events.addHandler(
            pin,
            "dragend",
            (args: any) => {
  ...
  AddPoi(pois: PoiPoint[]) {
    pois.forEach((poi) => {
      const pin: Microsoft.Maps.Pushpin = new Microsoft.Maps.Pushpin(
        new Microsoft.Maps.Location(poi.lat, poi.long),
        {
  ...
  private SetInfoBox(
    ...
    if (!this.infoBox) {
      this.infoBox = new Microsoft.Maps.Infobox(pin.getLocation(), {
        title: title,
